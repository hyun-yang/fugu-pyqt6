...
from fugu.util.Constants import UI, get_ai_provider_names
...
class GlobalSetting(QDialog):
    ...
    def create_info_label_style_group(self, layout):
        ...
        for i in range(len(self.info_color_editors)):
            self.info_color_editors[i].textChanged.connect(
                partial(self.handle_info_label_text_change, i)
            )

        for i, button in enumerate(self.info_buttons):
            button.setMaximumWidth(30)
            button.clicked.connect(
                partial(self.color_pick, color_types[i], self.info_color_editors[i])
            )
    ...
    def create_ai_code_view_style_group(self, mainLayout):
        ...
        self.ai_keys = [
            ...
            "font-size",
            "think-color",
            "think-background-color",
        ]
        self.ai_values = [
            self._settings.value(f"AI_Code_Style/{self.ai_keys[i]}", "")
            for i in range(len(self.ai_keys))
        ]

        self.ai_editors = [QLineEdit(self.ai_values[i]) for i in range(len(self.ai_labels))]
        self.ai_buttons = [
            QPushButton("...") if i in [0, 1, 4, 5, 2] else None for i in range(len(self.ai_labels))
        ]
    ...
    def create_common_label_style_group(self, mainLayout):
        ...
        self.commonLabelStyle_editors = [QLineEdit() for _ in self.commonLabelStyle_labels]
        self.commonLabelStyle_buttons = [
            QPushButton("...") if i != 0 and i != 3 else None
            for i in range(len(self.commonLabelStyle_labels))
        ]

        for i in range(len(self.commonLabelStyle_editors)):
            self.commonLabelStyle_editors[i].textChanged.connect(
                partial(self.handle_common_label_text_change, i)
            )
    ...
    def create_chat_title_bar_style_group(self, mainLayout):
        ...
        for i in range(len(self.chat_color_editors)):
            self.chat_color_editors[i].textChanged.connect(
                partial(self.handle_chat_title_bar_text_change, i)
            )

        for i, button in enumerate(self.chat_buttons):
            button.setMaximumWidth(30)
            button.clicked.connect(
                partial(self.color_pick, color_types[i], self.chat_color_editors[i])
            )
    ...
    def create_ai_provider_group(self, mainLayout):
        ...
        self.api_labels = ["OpenAI API", "Sakana API"]
