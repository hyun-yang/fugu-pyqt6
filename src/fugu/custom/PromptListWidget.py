...
from fugu.util.Constants import DATABASE_MESSAGE, UI, Constants
...
class PromptListWidget(QWidget):
    ...
    def initialize_ui(self):
        ...
        self._promptDB.setDatabaseName(str(user_data_base() / Constants.DATABASE_NAME))
        ...
        self.deleteButton = QPushButton(
            QIcon(Utility.get_icon_path("ico", "database--minus.png")), ""
        )
        ...
        self.copyButton = QPushButton(
            QIcon(Utility.get_icon_path("ico", "clipboard--arrow.png")), "Send"
        )
    ...
    def update_filter(self):
        ...
        self.filter_model.setFilterRegularExpression(
            QRegularExpression(
                self.searchTerm.text(), QRegularExpression.PatternOption.CaseInsensitiveOption
            )
        )
...
class PromptLazyLoadTableModel(QAbstractTableModel):
    ...
    def load_initial_data(self):
        ...
        if query.exec(f"SELECT id, title, prompt FROM {self.table_name}"):
            ...
            QMessageBox.critical(
                None, DATABASE_MESSAGE.DATABASE_TITLE_ERROR, f"Failed to load data: {error.text()}"
            )
    ...
    def reload_data(self):
        ...
        if query.exec(f"SELECT id, title, prompt FROM {self.table_name}"):
            ...
            QMessageBox.critical(
                None,
                DATABASE_MESSAGE.DATABASE_TITLE_ERROR,
