...
from fugu.util.Constants import MODEL_MESSAGE, UI, AgentPattern, AIProviderName, Constants
...
class AgentView(QWidget):
    ...
    def __init__(self, model):
        ...
        self._current_agent_pattern = Utility.get_settings_value(
            section="Agent_Pattern", prop="agent_pattern", default="Orchestrator", save=True
        )
        # The provider is fully derived from the active tab via
        # _PATTERN_TO_PROVIDER, so Agent_Pattern/llm in settings.ini is no
        # longer read. AIProviderName.SAKANA is just a safety fallback for
        # an unknown pattern; in normal operation the mapping covers it.
        self._current_llm = self._PATTERN_TO_PROVIDER.get(
            self._current_agent_pattern, AIProviderName.SAKANA.value
        )
    ...
    def update_main_model_list(self):
        ...
        try:
            ...
            current_model_combo = self.findChild(
                QComboBox, f"{self._current_agent_pattern}_ModelList"
            )
    ...
    def create_parameters_tab(self):
        ...
        self.tabs = QTabWidget()
        self.tabs.addTab(
            self.create_evaluator_tabcontent(AgentPattern.EVALUATOR.value),
            AgentPattern.EVALUATOR.value,
        )
        self.tabs.addTab(
            self.create_orchestrator_tabcontent(AgentPattern.ORCHESTRATOR.value),
    ...
    def on_tab_change(self, index):
        ...
        # Flush the tab choice promptly — QSettings would otherwise batch
        # the write and the next launch could pick up the old tab.
        self._settings.sync()
        self._sync_llm_to_pattern(self._current_agent_pattern)
        self.update_main_model_list()

    def _sync_llm_to_pattern(self, pattern):
        """Map the active agent-pattern tab to its provider and propagate
        that provider to (a) the in-memory cache used by
        ``update_main_model_list`` and (b) the presenter via
        ``current_llm_signal``. Idempotent.

        The provider is purely derived from the active tab via
        ``_PATTERN_TO_PROVIDER`` and is no longer persisted to settings.ini
        — only ``Agent_Pattern/agent_pattern`` is. A stale ``Agent_Pattern/llm``
        key in older settings.ini files is harmless and simply ignored.
        """
        provider = self._PATTERN_TO_PROVIDER.get(pattern)
        if not provider:
            return
        self._current_llm = provider
    ...
    def set_default_tab(self, name):
        ...
        if index != -1:
            if self.tabs.currentIndex() != index:
                # A real index change emits currentChanged, so on_tab_change
                # performs the provider sync and model-list refresh.
                self.tabs.setCurrentIndex(index)
                return
            active_pattern = name
        else:
            active_pattern = self.tabs.tabText(self.tabs.currentIndex())
        # If setCurrentIndex was a no-op (saved tab equals the default-
        # constructed index), currentChanged never fires; sync explicitly
        # so the presenter's self.llm matches the visible tab.
        self._current_agent_pattern = active_pattern
        self._settings.setValue("Agent_Pattern/agent_pattern", active_pattern)
        self._settings.sync()
        self._sync_llm_to_pattern(active_pattern)
        self.update_main_model_list()
    ...
    def create_evaluator_tabcontent(self, name):
        ...
        self.set_model_list(modelList, AIProviderName.SAKANA.value, name)
        ...
        max_tokensSpinBox.spin_box.setValue(
            int(
                Utility.get_settings_value(
                    section=f"{name}_Model_Parameter", prop="max_tokens", default="12800", save=True
                )
            )
        )
        ...
        max_retriesSpinBox.spin_box.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred
        )
        ...
        max_retriesSpinBox.spin_box.setValue(
            int(
                Utility.get_settings_value(
                    section=f"{name}_Model_Parameter", prop="max_retries", default="3", save=True
                )
            )
        )
    ...
    def create_orchestrator_tabcontent(self, name):
        ...
        max_tokensSpinBox.spin_box.setValue(
            int(
                Utility.get_settings_value(
                    section=f"{name}_Model_Parameter", prop="max_tokens", default="5000", save=True
                )
            )
        )
        max_tokensSpinBox.check_box.setChecked(True)
    ...
    def get_default_model_for_provider(self, provider_name):
        if provider_name == AIProviderName.OPENAI.value:
            return "gpt-5-mini"
        elif provider_name == AIProviderName.SAKANA.value:
            return "fugu-mini"
        else:
            return ""

    def set_model_list(self, modelList, provider, pattern_name):
        # The provider is fully derived from the active tab (see
        # _PATTERN_TO_PROVIDER) and is no longer persisted to settings.ini.
        # Each tab is built once at startup with its hard-coded provider,
        # so any write here would just reflect "whichever tab was built
        # last", which was the source of the historical drift bug.
        if provider == AIProviderName.OPENAI.value:
        ...
        elif provider == AIProviderName.SAKANA.value:
            ...
            raise ValueError(f"{MODEL_MESSAGE.MODEL_UNSUPPORTED}")
    ...
    def create_prompt_tab(self):
        ...
        self._prompt_list = PromptListWidget(
            table_name=Constants.AGENT_PROMPT_TABLE, db_connection_name="AgentPromptDBConnection"
        )
    ...
    def add_user_question(self, agentType, text):
        user_question = ChatWidget(agentType, text)
    ...
    def update_ui(self, result, stream):
        if stream:
            ...
            if agentWidget:
                ...
                agentWidget = ChatWidget(ChatType.AI)
                agentWidget.add_text(result)
    ...
    def create_orchestrator_args_openai(self, text, agent_llm, agent_pattern):
        ...
        stream = self.findChild(QCheckBox, f"{agent_pattern}_streamCheckbox").isChecked()

        max_tokens_spin_box = self.findChild(
            CheckSpinBox, f"{agent_pattern}_max_tokensSpinBox"
        ).spin_box
        ...
        temperature_spin_box = self.findChild(
            CheckDoubleSpinBox, f"{agent_pattern}_temperatureSpinBox"
        ).spin_box
        temperature_spin_box.value() if temperature_spin_box.isEnabled() else None

        orchestrator_prompt = self.findChild(
            QTextEdit, f"{AgentPattern.ORCHESTRATOR.value}_current_prompt"
        ).toPlainText()
        worker_prompt = self.findChild(
            QTextEdit, f"{AgentPattern.WORKER.value}_current_prompt"
        ).toPlainText()
        aggregator_prompt = self.findChild(
            QTextEdit, f"{AgentPattern.AGGREGATOR.value}_current_prompt"
        ).toPlainText()
        ...
        messages = [{"role": "user", "content": formatted_orchestrator_prompt}]
    ...
    def create_evaluator_args_sakana(self, text, agent_llm, agent_pattern):
        api_key = self._settings.value(f"AI_Provider/{agent_llm}")
        model = self.findChild(QComboBox, f"{agent_pattern}_ModelList").currentText()
        stream = self.findChild(QCheckBox, f"{agent_pattern}_streamCheckbox").isChecked()
        ...
        max_retries_spin_box = self.findChild(
            CheckSpinBox, f"{agent_pattern}_max_retriesSpinBox"
        ).spin_box
        max_retries = max_retries_spin_box.value() if max_retries_spin_box.isEnabled() else 1

        evaluator_prompt = self.findChild(
            QTextEdit, f"{AgentPattern.EVALUATOR.value}_current_prompt"
        ).toPlainText()
        generator_prompt = self.findChild(
            QTextEdit, f"{AgentPattern.GENERATOR.value}_current_prompt"
        ).toPlainText()
        task_prompt = self.findChild(
            QTextEdit, f"{AgentPattern.TASK.value}_current_prompt"
        ).toPlainText()
