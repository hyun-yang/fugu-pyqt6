...
평가만 해야 하며, 작업을 직접 해결하려고 시도하지 마세요.
모든 기준을 충족하고 더 이상 개선 제안이 없을 때만 "PASS"를 출력하세요.
다음 형식으로 간결하게 평가를 출력하세요.
...
당신의 목표는 <user input>을 기반으로 작업을 완료하는 것입니다. 이전 생성 결과에 대한
피드백이 있다면, 이를 반영하여 솔루션을 개선해야 합니다.

다음 형식으로 간결하게 답변을 출력하세요:

반드시 <thoughts>와 <response> Tag를 포함해야 합니다.

<thoughts>
[작업과 피드백에 대한 이해, 그리고 어떻게 개선할지에 대한 계획]
</thoughts>
...
### Orchestrator-Worker 워크플로우 프롬프트 예시

- Orchestrator-Worker 프롬프트 예시

```markdown
1) Orchestrator 프롬프트

다음 사용자 질문을 분석하고, 관련된 하위 질문 2개 또는 3개로 나누세요:

다음 형식으로 응답하세요:
{
    "analysis": "사용자 질문에 대한 이해와 생성한 하위 질문들의 근거를 자세히 설명하세요.",
    "tasks": [
        {
            "task": "하위 질문 1",
            "description": "이 하위 질문의 의도와 핵심 요점을 설명하세요."
        },
        {
            "task": "하위 질문 2",
            "description": "이 하위 질문의 의도와 핵심 요점을 설명하세요."
        }
        // 필요한 경우 추가 하위 질문을 포함하세요
    ]
}
최대 2개 또는 3개의 하위 질문을 생성하세요.
...
하위 질문에 답하는 철저하고 상세한 응답을 제공하세요.


3) Aggregator 프롬프트

아래의 질문과 응답을 요약한 최종 응답을 제공하세요.

- 하위 질문에 대한 응답은 가능한 한 포괄적이고 상세해야 합니다.
- 최종 보고서는 Markdown 형식으로 포괄적으로 제시되어야 합니다.
