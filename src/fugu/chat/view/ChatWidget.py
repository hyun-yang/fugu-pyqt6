...
class ChatWidget(QWidget):
    def __init__(self, chat_type: ChatType, text: str | None = None):
        super().__init__()
    ...
    def format_text_label(self):
        ...
        padding = Utility.get_settings_value(
            section="Common_Label_Style", prop="padding", default="5px", save=True
        )
        color = Utility.get_settings_value(
            section="Common_Label_Style", prop="color", default="#000000", save=True
        )
        font_size = Utility.get_settings_value(
            section="Common_Label_Style", prop="font-size", default="14px", save=True
        )
        font_family = Utility.get_settings_value(
            section="Common_Label_Style", prop="font-family", default="sans-serif", save=True
        )
        user_text_style = f"""
            QLabel {{
                padding: {padding};
    ...
    def create_top_widget(self):
        ...
        human_titlebar_background_color = Utility.get_settings_value(
            section="Chat_TitleBar_Style", prop="human_color", default="#dfccff", save=True
        )
        ai_titlebar_background_color = Utility.get_settings_value(
            section="Chat_TitleBar_Style", prop="ai_color", default="#d8fabe", save=True
        )
        ...
        copy_button.clicked.connect(
            lambda: QApplication.clipboard().setText(self.get_original_text())
        )
    ...
    def format_code_snippet(self, text):
        color = Utility.get_settings_value(
            section="AI_Code_Style", prop="color", default="#ccc", save=True
        )
        background_color = Utility.get_settings_value(
            section="AI_Code_Style", prop="background-color", default="#333333", save=True
        )
        font_size = Utility.get_settings_value(
            section="AI_Code_Style", prop="font-size", default="14px", save=True
        )
        font_family = Utility.get_settings_value(
            section="AI_Code_Style", prop="font-family", default="monospace", save=True
        )
